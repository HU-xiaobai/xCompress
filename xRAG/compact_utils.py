...
def create_prompt_xRAG(example, iteration, iter_idx, document_input, prev_summary, prev_eval, tokenizer, eos_token="<|endoftext|>", add_generation_prompt=False):
    if iter_idx == 0:
        instruction = "[INST] Refer to the background document and answer the questions:"

        prompt = f"{instruction}\n\nSource documents: {document_input}\n\nQuestion: {example['question']}[/INST] The answer is:"
    else:
        instruction = "[INST] Refer to the background document and answer the questions:"
...
def parse_output_without_sentence_xRAG(text):
    sections = {}
    sections['summary'] = text
    sections['eval'] = "[COMPLETE]"
