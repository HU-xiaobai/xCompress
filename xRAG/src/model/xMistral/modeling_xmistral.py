...
class XMistralForCausalLM(MistralForCausalLM):
    ...
    def prepare_inputs_embeds(self,input_ids,retrieval_embeds):
        ...
        return inputs_embeds,retrieval_embeds
    ...
    def forward(
        ...
        **kwargs,
        ...
        if inputs_embeds is not None:
            ...
            # allow both inputs_embeds and retrieval_embeds together
            at_the_beginning_of_generation = True

        if not at_the_beginning_of_generation:
            ## a single forward
            if retrieval_embeds is not None:
                inputs_embeds,retrieval_embeds_xRag = self.prepare_inputs_embeds(input_ids,retrieval_embeds)
                input_ids = None
    ...
    def generate(
        ...
        **kwargs,
        ...
        inputs_embeds=None
        self.last_retrieval_embeds = None  # <- 清空上一次生成的缓存

        if retrieval_embeds is not None:
            inputs_embeds,retrieval_embeds_xRag = self.prepare_inputs_embeds(input_ids,retrieval_embeds)
            self.last_retrieval_embeds = retrieval_embeds_xRag  # <- 缓存 projector 后的结果
