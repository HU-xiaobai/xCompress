        ...
        if args.batch_decoding:
            ...
            for idx in tqdm(range(0, len(data_examples), args.batch_size)):
                ...
                for seg_idx in tqdm(range(0, max(len(ex['documents_list']) for ex in batch_examples), args.segment_size)):
                    ...
                    prompt_len = inputs_batch['input_ids'].shape[-1]
                    num_beams = 5  # 跟 generate 里保持一致

                    for ae_idx, ae in enumerate(active_examples):
                        iteration = ae["iteration"]

                        # 1. 取出该 example 对应的 5 条序列
                        start = ae_idx * num_beams
                        end = start + num_beams
                        seqs = outputs.sequences[start:end]  # Tensor 列表
                        # （可选）beam search 自带的 sum(log-prob)
                        beam_scores = outputs.sequences_scores[start:end].tolist()

                        # 2. 解码成完整文本（summary+Evaluation）
                        full_texts = [
                            tokenizer.decode(seq[prompt_len:], skip_special_tokens=True).strip()
                            for seq in seqs
                        ]

                        # 3. 只提取纯 summary（用于 space error），但解析要用完整文本
                        summaries = []
                        ...
                        # 用 summary 计算空间误差（注意变量名用 summary 而不是 txt）
                        errors = [
                            compute_space_error(pca, reader_model, reader_tokenizer, summary)
                            for summary in summaries
                        ]

                        # 选误差最小的 beam
                        best_idx = int(np.argmin(errors))
                        best_full = full_texts[best_idx]  # 完整文本（含 Evaluation）
                        best_sum = summaries[best_idx]  # 仅 summary（给 PCA 用）
                        ...
                        # 记录
                        iteration['output'] = best_full  # 保存完整输出
                        ...
                        try:
                            ...
                            print(f"ERROR parsing best beam for example {ae_idx}: {e}")
                            continue

                        iteration.update(parsed_sections)

                        # 关键：用 get 提供默认值，避免 KeyError('eval')
